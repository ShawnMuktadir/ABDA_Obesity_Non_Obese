...
png("shawn_matrix.png", width = 1200, height = 1200)
corrplot(cor_matrix, method = "color", type = "upper", order = "hclust",
...
subdataset <- cleaned_data[sample_indices, ]  
...
head(standarized_stratified_data)
```
...
```{r}
# using standarized_stratified_data as dataset
 #fit_model1 <- brm(
  # formula = formula1,
...
```{r}
model5 <- readRDS("hier_model_10percdata.rds")
summary(model5)
...
# Extract posterior samples for model3
posterior_samples <- as.array(model3)
...
```{r}
# Trace Plots for Each Group of model3
...
```{r}
loo_model1 <- loo(model1, moment_match = TRUE, cores = 1)
loo_model2 <- loo(model3, moment_match = TRUE, cores = 1)

loo_compare(loo_model1, loo_model2)

cat("[Info] Model comparison and validation steps completed successfully.\n")

model1 <- load_or_recompile("fit_model1.rds")
model3 <- load_or_recompile("hierarchical_model1.rds")
...
# Optimize memory and enable cached compilation
options(future.globals.maxSize = 5 * 1024^3)
rstan_options(auto_write = TRUE)  
options(mc.cores = parallel::detectCores())  

# Function to properly reload and recompile a model
reload_and_recompile_model <- function(model_path) {
  if (file.exists(model_path)) {
    message("Loading precompiled model from file...")
    model <- readRDS(model_path)  
  } else {
    stop("Model file not found! Please refit and save.")
  }

  if (!brms::is.brmsfit(model)) {
    stop("Loaded model is invalid! Please refit and save a correct model.")
  }
  
  # Force recompilation to fix C++ model issues
  model <- update(model, recompile = TRUE)  
  return(model)
}

# Reload and fully recompile models
model1 <- reload_and_recompile_model("fit_model1.rds")
model3 <- reload_and_recompile_model("hierarchical_model1.rds")

# Function to compute LOO with automatic issue handling
compute_loo <- function(model, model_name) {
  message(paste("Computing LOO for", model_name, "..."))

  loo_result <- loo(model, cores = parallel::detectCores(), moment_match = TRUE)

  if (any(loo_result$diagnostics$pareto_k > 0.7)) {
    message(paste("High Pareto k detected in", model_name, "- Running reloo..."))
    loo_result <- loo(model, cores = parallel::detectCores(), reloo = TRUE)
  } else {
    message(paste("LOO computed successfully for", model_name))
  }
...
loo_model1 <- compute_loo(model1, "Model 1")
loo_model2 <- compute_loo(model3, "Model 2")
...
loo_model1 <- loo(model1, moment_match = TRUE, cores = 2)
loo_model2 <- loo(model3, moment_match = TRUE, cores = 2)
